...
Multiple orchestrators for parallel collation.
"""
import threading
import time
import json
...
from minimal_dataset.monitored_queue import MonitoredQueue
from minimal_dataset.metrics import MetricsTracker
...
class DataLoader:
    ...
    def _orchestrator(self):
        ...
        while not self._stop_event.is_set():
            try:
                ...
                if done >= self.num_workers and self.staging_queue.qsize() == 0:
                    break
    ...
    def __iter__(self):
        ...
        for _ in range(self.num_orchestrators):
    ...
    def _cleanup(self):
        ...
        if self.metrics_dir:
            ...
            path = os.path.join(
                self.metrics_dir,
                f"metrics_w{self.num_workers}_o{self.num_orchestrators}_bs{self.batch_size}.json"
            )
